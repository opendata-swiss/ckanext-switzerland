...
from processors import SwissDCATRDFParser, SHACL_DIR
...
class SwissDCATRDFHarvester(DCATRDFHarvester):
    ...
    def validate_config(self, source_config):
        ...
        if 'shacl_validation_file' in source_config_obj:
            shapefilepath = os.path.join(
                SHACL_DIR, source_config_obj['shacl_validation_file'])
    ...
    def _set_gather_config(self, raw_config):
        ...
        config['ech_validation'] = bool(config['shacl_validation_file'])
        ...
        while next_page_url:
            ...
            try:
                rc = parser.parse(
                    ...
                    ech_validation=gather_config['ech_validation'],
                    shacl_file=gather_config['shacl_validation_file'],
                )
                if rc:
                    shacl_error_dict = parser.shaclresults()
                    for node in shacl_error_dict.keys():
                        for error in shacl_error_dict[node]:
